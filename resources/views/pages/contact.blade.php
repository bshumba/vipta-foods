...
@section('content')
    <section class="bg-vipta-cream">
        <div
            class="mx-auto grid min-h-[610px] max-w-7xl items-center gap-12 px-5 py-16 sm:px-8 min-[900px]:grid-cols-[0.92fr_1fr] lg:min-h-[680px] lg:px-12 lg:py-24">
            <div class="max-w-2xl">
    ...
    <section class="bg-vipta-cream py-16 lg:py-24">
        <div class="mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 min-[900px]:grid-cols-[0.92fr_1fr] lg:px-12">
            <div x-data="{ submitted: false, selectedEnquiry: '', descriptions: {{ \Illuminate\Support\Js::from($enquiryTypeDescriptions) }} }"
                class="rounded-lg border border-vipta-border bg-white p-6 shadow-[var(--shadow-vipta-soft)] sm:p-8 lg:p-10">
                <x-site.section-heading :eyebrow="$form['eyebrow'] ?? ''" :heading="$form['heading'] ?? ''"
                    :body="$form['intro'] ?? ''" />

                <form class="mt-8 grid gap-5" @submit.prevent="submitted = true">
                    <div>
                        <label for="name"
                            class="text-sm font-semibold text-vipta-green">{{ $form['name_label'] ?? 'Full name' }}</label>
                        <input id="name" name="name" type="text" required
                            class="mt-2 w-full rounded-lg border border-vipta-border bg-vipta-cream px-4 py-3 text-base text-vipta-ink outline-none transition focus:border-vipta-green focus:ring-2 focus:ring-vipta-gold/40">
                    </div>

                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            <label for="phone"
                                class="text-sm font-semibold text-vipta-green">{{ $form['phone_label'] ?? 'Phone / WhatsApp' }}</label>
                            <input id="phone" name="phone" type="tel"
                                class="mt-2 w-full rounded-lg border border-vipta-border bg-vipta-cream px-4 py-3 text-base text-vipta-ink outline-none transition focus:border-vipta-green focus:ring-2 focus:ring-vipta-gold/40">
                        </div>
                        <div>
                            <label for="email"
                                class="text-sm font-semibold text-vipta-green">{{ $form['email_label'] ?? 'Email address' }}</label>
                    ...
                    <div>
                        <label for="enquiry_type"
                            class="text-sm font-semibold text-vipta-green">{{ $form['type_label'] ?? 'Enquiry type' }}</label>
                        <select id="enquiry_type" name="enquiry_type" required x-model="selectedEnquiry"
                            class="mt-2 w-full rounded-lg border border-vipta-border bg-vipta-cream px-4 py-3 text-base text-vipta-ink outline-none transition focus:border-vipta-green focus:ring-2 focus:ring-vipta-gold/40">
                        ...
                        <p x-cloak x-show="selectedEnquiry" x-transition.opacity.duration.200ms
                            x-text="descriptions[selectedEnquiry]"
                            class="mt-3 rounded-lg border border-vipta-border bg-vipta-cream px-4 py-3 text-sm leading-6 text-vipta-muted">
                        </p>
                    </div>

                    <div>
                        <label for="message"
                            class="text-sm font-semibold text-vipta-green">{{ $form['message_label'] ?? 'Message' }}</label>
                        <textarea id="message" name="message" rows="6" required
                            placeholder="{{ $form['message_placeholder'] ?? '' }}"
                            class="mt-2 w-full resize-y rounded-lg border border-vipta-border bg-vipta-cream px-4 py-3 text-base text-vipta-ink outline-none transition focus:border-vipta-green focus:ring-2 focus:ring-vipta-gold/40"></textarea>
                    </div>

                    <button type="submit"
                        class="rounded-lg bg-vipta-green px-6 py-4 text-center text-sm font-semibold text-white shadow-[var(--shadow-vipta-soft)] transition hover:bg-vipta-leaf focus:outline-none focus:ring-2 focus:ring-vipta-gold focus:ring-offset-2 focus:ring-offset-white">
                        {{ $form['submit_label'] ?? 'Send Enquiry' }}
                    </button>

                    <div x-cloak x-show="submitted" x-transition
                        class="rounded-lg border border-vipta-gold/70 bg-vipta-cream px-4 py-4 text-sm leading-6 text-vipta-muted">
                ...
                @foreach ($contactCards as $card)
                    ...
                    <x-site.text-card variant="cream">
                        ...
                        <h3 class="mt-3 break-words font-display text-lg font-semibold text-vipta-green sm:text-xl">
                            ...
                            @elseif (in_array($valueKey, ['phone', 'phone_us', 'phone_zim'], true))
                                @php
                                    $cleanPhone = preg_replace('/\s+/', '', $value);
                                @endphp
                                <a href="tel:{{ $cleanPhone }}" class="transition hover:text-vipta-leaf">{{ $value }}</a>
                            @elseif ($valueKey === 'whatsapp')
                                @php
                                    $cleanWa = preg_replace('/\D+/', '', $value);
                                @endphp
                                <a href="https://wa.me/{{ $cleanWa }}" target="_blank" rel="noopener noreferrer"
                                    class="transition hover:text-vipta-leaf">{{ $value }}</a>
                            @else
                                {{ $value }}
                            @endif
                        </h3>
