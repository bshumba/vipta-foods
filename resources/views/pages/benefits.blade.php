...
@section('content')
    <section class="bg-vipta-cream">
        <div
                ...
                <div class="mt-9 grid gap-3 sm:grid-cols-3">
                    @foreach (($hero['stats'] ?? []) as $stat)
                        <div class="rounded-lg border border-vipta-border bg-vipta-paper px-5 py-4 shadow-sm">
                            ...
                            <p class="mt-1 text-xs font-bold uppercase tracking-[0.12em] text-vipta-earth">
                                {{ $stat['label'] ?? '' }}
                            </p>
                        </div>
                    @endforeach
            ...
            <div class="relative">
                <div
                    class="overflow-hidden rounded-2xl border border-vipta-border bg-vipta-paper p-4 shadow-[var(--shadow-vipta-soft)]">
                    <img src="{{ asset($hero['image'] ?? 'images/vipta/story-matohwe-harvest.png') }}"
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    @foreach ($compounds as $compound)
                        <article class="rounded-lg border border-vipta-border bg-vipta-cream p-6 shadow-sm">
                            <p class="text-xs font-bold uppercase tracking-[0.14em] text-vipta-earth">Bioactive compound</p>
                            <h3 class="mt-3 font-display text-2xl font-semibold text-vipta-green">{{ $compound['name'] ?? '' }}
                            </h3>
                            <p class="mt-3 text-sm leading-6 text-vipta-muted">{{ $compound['summary'] ?? '' }}</p>
                            @if (filled($compound['source_url'] ?? null))
                                <a href="{{ $compound['source_url'] }}" target="_blank" rel="noopener noreferrer"
    ...
    <section class="bg-vipta-cream py-16 lg:py-24">
        <div class="mx-auto grid max-w-7xl items-start gap-10 px-5 sm:px-8 min-[900px]:grid-cols-[0.8fr_1fr] lg:px-12">
            <div>
                <x-site.section-heading :eyebrow="$wellness['eyebrow'] ?? ''" :heading="$wellness['heading'] ?? ''"
                    :body="$wellness['body'] ?? ''" />
                @if (filled($wellness['source_url'] ?? null))
                    <a href="{{ $wellness['source_url'] }}" target="_blank" rel="noopener noreferrer"
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                @foreach (($wellness['items'] ?? []) as $item)
                    <article class="rounded-lg border border-vipta-border bg-vipta-paper p-6 shadow-sm">
                        <span
                            class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-vipta-green font-display text-lg font-semibold text-white">{{ $loop->iteration }}</span>
    ...
    <section class="bg-vipta-cream py-16 lg:py-24">
        <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            <x-site.section-heading :eyebrow="$sexualIntro['eyebrow'] ?? ''" :heading="$sexualIntro['heading'] ?? ''"
                :body="$sexualIntro['body'] ?? ''" align="center" />
                ...
                @foreach ($sexualBenefits as $benefit)
                    <article class="rounded-lg border border-vipta-border bg-vipta-paper p-7 shadow-[var(--shadow-vipta-soft)]">
                        ...
                        <p class="mt-5 text-xs font-bold uppercase tracking-[0.12em] text-vipta-earth/80">
                            {{ $benefit['source_label'] ?? 'Traditional knowledge' }}
                        </p>
                    </article>
                @endforeach
            </div>
    ...
    <!-- @if ($sourceItems->isNotEmpty())
                    <section class="border-y border-vipta-border bg-vipta-paper py-16 lg:py-24">
                        <div class="mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 min-[900px]:grid-cols-[0.72fr_1fr] lg:px-12">
                            <x-site.section-heading :eyebrow="$sourcesIntro['eyebrow'] ?? ''" :heading="$sourcesIntro['heading'] ?? ''"
                                :body="$sourcesIntro['body'] ?? ''" />
    ...
    <!-- <x-site.cta-section :heading="$cta['heading'] ?? ''" :body="$cta['body'] ?? ''"
                :primary-label="$cta['primary_label'] ?? 'Explore Products'" :primary-route="$cta['primary_route'] ?? 'products'"
                :secondary-label="$cta['secondary_label'] ?? 'Order / Enquire'"
                :secondary-route="$cta['secondary_route'] ?? 'contact'" variant="panel" class="bg-vipta-sage" /> -->
@endsection